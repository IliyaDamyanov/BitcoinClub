namespace BitcoinClub.Services.Events;
...
public sealed class EventsService : IEventsService
{
    ...
    /// <c>AddHttpClient&lt;IEventsService, EventsService&gt;()</c> in Program.cs).
    /// </summary>
    public EventsService(HttpClient http, IMemoryCache cache, ILogger<EventsService> logger)
    {
        _http = http;
        _cache = cache;
        _logger = logger;
    ...
    /// <inheritdoc/>
    public async Task<IReadOnlyList<EventViewModel>> GetUpcomingAsync(int maxCount = 6)
    {
        if (_cache.TryGetValue(AppConstants.Calendar.CacheKey, out IReadOnlyList<EventViewModel>? cached) && cached != null)
            return cached;
        ...
        try
        {
            ...
            if (calendar is null) return Array.Empty<EventViewModel>();
            ...
            var from  = new CalDateTime(DateTime.UtcNow.AddDays(-1));
            var until = DateTime.UtcNow.AddMonths(3);
            ...
            _cache.Set(AppConstants.Calendar.CacheKey, (IReadOnlyList<EventViewModel>)events, TimeSpan.FromMinutes(30));
